...
public sealed class PlanetRosterMutationTests
{
    ...
    private static (Planet Planet, Guid OwnerId, Guid ShipId) PlanetWithRosterShip()
    {
        ...
        planet.Apply(new PlanetCreated("P", Guid.NewGuid(), 1000, 5, 1000, 1000, 0m, 0m, 0m));
        planet.Apply(new PlanetColonized(ownerId, 0, 0, _t0));
        planet.Apply(new ShipConstructionQueued(shipId, ShipType.CargoVessel, _t0, 0m, 60m));
        planet.Apply(new ShipConstructionStarted(shipId, _t0, _t0.AddSeconds(60)));
        planet.Apply(new ShipCompleted(shipId, _t0.AddSeconds(60)));
    ...
    public void RemoveShipsFromRosterRemovesExactlyThoseShips()
    {
        ...
        var @event = planet.RemoveShipsFromRoster(fleetId, [shipId], _t0.AddSeconds(120));
        planet.Apply(@event);
    ...
    public void RemoveShipsFromRosterWithUnknownIdThrows()
    {
        ...
        Assert.Throws<InvalidOperationException>(
            () => planet.RemoveShipsFromRoster(Guid.NewGuid(), [Guid.NewGuid()], _t0));
    }
    ...
    {
        var (planet, ownerId, shipId) = PlanetWithRosterShip();
        planet.Apply(planet.RemoveShipsFromRoster(Guid.NewGuid(), [shipId], _t0.AddSeconds(120)));

        var returned = new RosterShip(shipId, ShipType.CargoVessel, _t0.AddSeconds(60), ownerId);
        planet.Apply(planet.ReturnShipsToRoster(Guid.NewGuid(), [returned], _t0.AddSeconds(200)));
