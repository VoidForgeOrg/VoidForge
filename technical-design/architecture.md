...
### Same-Stream Concurrency (#27, #39)

Multiple paths append to one `Planet` event stream concurrently: the scheduled completion handlers (`CompleteBuildingConstructionHandler`, `CompleteShipConstructionHandler`) and the HTTP command endpoints (`BuildingEndpoints.Place`, `ShipEndpoints.Queue`/`Cancel`). A Shipyard runs up to `ShipyardParallelBuilds` (3) builds at once, so several `CompleteShipConstruction` messages can share an identical `CompletesAt` and fire in the same `DurabilityAgent` poll; a player command can also land within a poll of a scheduled completion. Under `EventAppendMode.Quick` a plain `session.Events.Append` does no version check, so two appends racing to the same next stream version collided on Postgres `23505` (`pk_mt_events_stream_and_version`) — surfaced first in the end-to-end ship tests (`ShipConstructionCompletionTests`). An HTTP loser returned **500**; a scheduled loser, with no retry policy, could dead-letter — permanently losing the completion and leaving a ship/building stuck `Active`/`UnderConstruction` with a stalled queue.

**Resolution (#39): optimistic concurrency + retry.** Every planet-stream append site now uses `session.Events.FetchForWriting<Planet>(id)`, which loads the aggregate *and* arms Marten's optimistic-concurrency guard from the fetched version. It works under `Quick` mode: a conflict surfaces as `JasperFx.Events.EventStreamUnexpectedMaxEventIdException` (a subclass of `JasperFx.ConcurrencyException`), transformed from the `23505`. A losing append therefore fails cleanly on commit instead of racing to a duplicate version.
...
- **Fleet recall (implemented #73):** `POST /api/fleets/{id}/cancel` turns an in-transit fleet around to head back to its origin. Following the aggregate pattern (no saga — see the #49 supersession above), it appends a single `FleetRecalled` event to the `Fleet` stream carrying a synthesized return `TravelPlan` (`ArrivesAt = now + elapsed`, one leg back to the origin — synthesized directly rather than via `ITravelPlanner`, since an in-transit fleet has no live position) and schedules a fresh `CompleteFleetArrival` at the return time. `Apply(FleetRecalled)` sets `Mission = Move` and `DestinationPlanetId = OriginPlanetId` (cargo and colony ship untouched) so the return arrives `Stationed` at origin with nothing delivered, and stamps a `RecalledAt` marker (409 "already returning" on a second recall). The **originally-scheduled** `CompleteFleetArrival(oldArrivesAt)` goes stale and no-ops via `Fleet.Arrive`'s validate-on-arrival check (`ArrivesAt == at`), exactly like a stale/duplicate arrival today — no outbox cancellation (ADR 0001). The historical saga sketch (a `CancelFleet` message creating a return-journey saga with a `NotFound` handler ignoring the late `FleetArrived`) predates #49 and is superseded by this.
