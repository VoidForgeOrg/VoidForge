...
| D10 | **Balance values come from a `Balance` config section, test-overridable.** ~~Config-backed statics~~ **Superseded during #26 implementation:** the *static* mechanism is unsafe (unit + integration tests share one process; xUnit interleaves them, so a boot-configured static bleeds test durations into unit tests) **and** replay-unsafe (a startup-loaded static makes replay depend on current config, not the value in effect when construction started). Implemented instead as `IOptions<BalanceOptions>` injected at the endpoint, with the derived drain + `CompletesAt` carried on `BuildingConstructionStarted` into aggregate state — so `Apply`/`RebaseRates` stay pure and replay reproduces the original economics exactly. Honors D10's intent (config-section balance, test-overridable, no DI in `Apply`). | Marten `Apply` can't use DI; carrying the drain on the event means `RebaseRates` re-sums from slot state with no spec lookup, and enables short-duration end-to-end tests + recompile-free tuning without a mutable static |
